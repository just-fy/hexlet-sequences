...
export const reduce = (func, acc, htmlList) => {
  ...
  const item = head(htmlList);
  return reduce(func, func(item, acc), tail(htmlList));
};

export const emptyTagsCount = (tagName, htmlList) => {
  const func = (element, acc) => (is(tagName, element) && (value(element) === '')) ? acc + 1 : acc;
  return reduce(func, 0, htmlList);
};
// END
